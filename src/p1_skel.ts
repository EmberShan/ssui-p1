...
class UIClass {
  ...
  set context(v: CanvasRenderingContext2D) {
    this._context = v;
  }
  ...
  constructor(canvasTagID: string) {
    ...
    this._canvas = validate(
      document.getElementById(canvasTagID),
      isHTMLCanvasElement,
      `Can't find a canvas element with id: "${canvasTagID}"`
    );
    this._context = validate(
...
function pickPositionInBox(
  boundW: number,
  boundH: number,
  diam: number
): [number, number] {
  return [
    Math.round(randomInRange(diam, boundW - diam)),
...
class FittsTrialRecord {
  ...
  toString(): string {
    ...
    return (
      `${twoPlaces(this.startTime)}, ${twoPlaces(this.endTime)}, ` +
...
class FittsTestUI extends UIClass {
  ...
  get theReticle(): Reticle {
    return this._theReticle;
  }

  protected _theTarget!: Target;
  get theTarget(): Target {
  ...
  get theBackground(): BackgroundDisplay {
    return this._theBackground;
  }
  ...
  override buildUIObjects() {
    ...
    this.childObjects.push(
      (this._theBackground = new BackgroundDisplay(
        this.canvas.width,
        this.canvas.height,
        this
      ))
  ...
  override configure(newState: string) {
    ...
    switch (this.currentState) {
      ...
      case "begin_trial":
        // === YOUR CODE HERE ===
        // updating the trial number which is incremented in the newTrial() function
        this.theBackground.msg1 =
          "Trial #" + this.trialCount + " of " + this.MAX_TRIALS;
      ...
      case "in_trial":
        // === YOUR CODE HERE ===
        // showing the target and hiding the reticle
      ...
      case "ended":
        // === YOUR CODE HERE ===
        // hiding the target and reticle and display the final message
        this.theTarget.visible = false;
        this.theReticle.visible = false;
      ...
      default:
        throw Error(
          `Unknown UI state ${newState} passed to FittsTestUI.configure()`
        );
    }
  }
  ...
  newTrial() {
    ...
    if (this.trialCount > this.MAX_TRIALS) {
      ...
      // otherwise we have a normal trial...
      ...
      // === YOUR CODE HERE ===
      // start each new trial with the state "begin_trial"
      this.configure("begin_trial");
      // setting the new randomized center position for both reticle and target
      // for target, also set a new random diameter
  ...
  get trialData(): FittsTrialRecord[] {
  ...
  recordTrialEnd(endLocX: number, endLocY: number, endSz: number) {
    ...
    const dist = Math.hypot(
      this.lastTrialLocation[0] - endLocX,
      this.lastTrialLocation[1] - endLocY
    );

    // create a record for the trial and append to the data
    const rec: FittsTrialRecord = new FittsTrialRecord(
      this.lastStartTime,
      endT,
      dist,
      endSz
...
class Target extends ScreenObject {
  ...
  override get parentUI(): FittsTestUI {
    return this._parentUI;
  }
  ...
  newGeom(newCentX: number, newCentY: number, newDiam?: number) {
    // === YOUR CODE HERE ===
    // set the new center position based on the changed width and height
    this.centerX = newCentX;
    this.centerY = newCentY;
    // set the diameter only for targets (reticles won't change the diameter)
    if (newDiam) {
      this.diam = newDiam;
  ...
  override pickedBy(ptX: number, ptY: number): boolean {
    ...
    if (
      Math.sqrt((ptX - this.centerX) ** 2 + (ptY - this.centerY) ** 2) <=
      this.radius
    ) {
      return true;
    }
  ...
  override handleClickAt(ptX: number, ptY: number): boolean {
    ...
    if (
      this._parentUI.currentState === "in_trial" &&
      this._visible &&
...
class Reticle extends Target {
  ...
  override draw(ctx: CanvasRenderingContext2D): void {
    // === YOUR CODE HERE ===
    // only run this if reticle should be visible
    // meaning the state is "begin_trial"
    if (this._parentUI.theReticle._visible) {
      // large outer circle
      ctx.beginPath();
      ctx.arc(
        this.centerX,
      ...
      // cross lines
      const outer_radius = Reticle.RETICLE_DIAM / 2;
      ctx.beginPath();
      ctx.moveTo(this.centerX - outer_radius, this.centerY);
      ctx.lineTo(this.centerX + outer_radius, this.centerY);
      ctx.stroke();

      ctx.beginPath();
      ctx.moveTo(this.centerX, this.centerY - outer_radius);
      ctx.lineTo(this.centerX, this.centerY + outer_radius);
      ctx.stroke();
    }
  }
  ...
  override pickedBy(ptX: number, ptY: number): boolean {
    // === YOUR CODE HERE ===
    // the distance of the click to the center should be equal to or smaller than the radius of the inner circle
    if (
      Math.sqrt((ptX - this.centerX) ** 2 + (ptY - this.centerY) ** 2) <=
      Reticle.RETICLE_INNER_DIAM / 2
    ) {
  ...
  override handleClickAt(ptX: number, ptY: number): boolean {
    // === YOUR CODE HERE ===
    if (
      this._parentUI.currentState === "begin_trial" &&
      this._visible &&
      this.pickedBy(ptX, ptY)
    ) {
...
class BackgroundDisplay extends ScreenObject {
  ...
  override draw(ctx: CanvasRenderingContext2D): void {
    ...
    // === YOUR CODE HERE ===
    // drawing the messages that are set in FittsTestUI configure switch cases
    // increment the line position by adding the fontHeight
    // and make sure the line does not go outside the canvas width
    ctx.fillText(this._msg1, xpos, ypos, this._parentUI.canvas.width);
    ctx.fillText(
      this._msg2,
      xpos,
      ypos + fontHeight,
      this._parentUI.canvas.width
    );
    ctx.fillText(
      this._msg3,
      xpos,
